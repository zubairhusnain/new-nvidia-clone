...
(function($, window, document, undefined) {
	...
	Plugin.prototype.init = function() {
		...
		if (NVIDIAGDC.AdaptiveForm.starfleetAuthRequired === 'true') {	
			var isEditMode = document.body.getAttribute('data-edit-mode') === 'true';
			if(!isEditMode) {
				//Check if the form is in a modal			
				var modalForm = $('.nv-modal-container').find('.aemformcontainer');
				if(modalForm.length > 0){
					NVIDIAGDC.AdaptiveForm.checkStarfleetLogin(true, $('.nv-modal-container').parent().parent().attr('id'), false);
				} else {
					NVIDIAGDC.AdaptiveForm.checkStarfleetLogin(false, '', false);
				}
			}
		...
		if (NVIDIAGDC.AdaptiveForm.autoFormFill === 'true' && NVIDIAGDC.AdaptiveForm.syncToMarketo === 'true') {
			this.autoFormFill();
		}

		//this.btnSubmit();	
		var doNotAllowDuplicate = NVIDIAGDC.AdaptiveForm.doNotAllowDuplicateSubmission;
		guideBridge.on("elementValueChanged", function(e, focuspayload) {
			...
			if (component.jsonModel.name === "userEmail") {
				...
				if (doNotAllowDuplicate === "true") {
					$.ajax({
						"url": NVIDIAGDC.AdaptiveForm.aemFormsApiHost + "/aem-forms-api/api/v1/submitdata/check/email?form-name=" + NVIDIAGDC.AdaptiveForm.formName + "&email=" + encodeURIComponent(guideBridge.resolveNode('userEmail').value),
						"async": false,
						"crossDomain": true,
	...
	// Send dynamic height to parent
	Plugin.prototype.sendHeightToParent = function() {
		const aemForm = document.querySelector('.aemform');
		const nvPageBodyRoot = document.querySelector('.nv-page-body > .root');

		if (!aemForm || !window.frameElement) {
			return; // Exit if '.aemform' is not found or not running in an iframe
		}

		const childHeight = () => {
			const height = aemForm.scrollHeight;
			if (nvPageBodyRoot) {
				nvPageBodyRoot.style.marginTop = '0';
			}

			const message = { height: height + 30 };
			window.parent.postMessage(message, "*");
		};

		childHeight(); // Calculate and send initial height

		window.addEventListener('resize', childHeight); // Update height on resize
	};
	...
	Plugin.prototype.selectStyled = function() {
		...
		$('.prefill-login-clear').on("click", function(e) {
			e.preventDefault();
			if ($('.rememberMe input').length > 0 && localStorage.getItem('nv-web-data') !== null) {
				$(".rememberMe input").prop("checked", false);
				NVIDIAGDC.AdaptiveForm.rememberMe = false;
				localStorage.removeItem('nv-web-data');				
			}
			guideBridge.reset();
			...
			if (lastLogin !== undefined && lastLogin !== null && lastLogin !== "") {
				...
				if(Cookies.get('isAemFormLoginRedirect') !== undefined || NVIDIAGDC.AdaptiveForm.starfleetAuthRequired) {
		...
		$('#acc-login-link').on("click", function(e) {
			e.preventDefault();			
			var loginSessionTokens = NVIDIAGDC.AdaptiveForm.getLoggedInUserData();
      
			if (loginSessionTokens === "undefined" || 
			loginSessionTokens === null || 
			new Date() > new Date(loginSessionTokens.expiration)) {

				//check modalId is present
				var modalId = $('.nv-modal-container').parent().parent().attr('id');
				if(modalId !== undefined && modalId !== null && modalId !== ""){
					NVIDIAGDC.AdaptiveForm.checkStarfleetLogin(true, modalId, true);
	...
	Plugin.prototype.setNcidVal = function() {
		...
		function getParameterByName(name) {
			...
			var results = (window.frameElement != null ? regex.exec(window.parent.location.href) : regex.exec(window.location.href));
			if (results == null) {
				return "";
			} else {
	...
	Plugin.prototype.getLoggedInUserData = function() {
		return NVIDIAGDC.AdaptiveForm.getLoggedInUserData();
	}
	
	NVIDIAGDC.AdaptiveForm.getLoggedInUserData = function() {
		var lastLogin = window.localStorage.getItem('lastLoginData');
		if (lastLogin !== undefined && lastLogin !== null && lastLogin !== "") {
		...
		if (userProfileData !== null) {
			if (new Date() <= new Date(userProfileData.expiration)) {
				if (NVIDIAGDC !== undefined) {
					...
					// add check for form-login-message
					var loginMessage = document.getElementById('form-login-message');
                    if (loginMessage !== undefined && loginMessage !== null) {
                        loginMessage.style.display = 'none';
                    }
				}

				if ($('.rememberMe input').length > 0) {
					 $('.rememberMe label').eq(1).text('Remember me when logged out');
					if (localStorage.getItem('nv-web-data') !== null) {
						NVIDIAGDC.AdaptiveForm.rememberMe = true;
						$('.rememberMe input').prop('checked', true);
					}
				}

				$.ajax({
					type: "GET",
					url: NVIDIAGDC.AdaptiveForm.eksApiServerHost + '/services/eupservice/v3/user/eup-profile',
					crossDomain: true,
					headers: {
						'Accept': 'application/json',
						'Content-Type': 'application/json',
						'Client': userProfileData['client_id'],
					...
					success: function(data) {
						...
						if (data.userEmail !== undefined) {
							...
							if (guideBridge.resolveNode("userEmail") !== undefined && guideBridge.resolveNode("userEmail") !== null) {
								...
								NVIDIAGDC.AdaptiveForm.emailIdUpdatedWithAutoPrefill = true;
								NVIDIAGDC.AdaptiveForm.skipEupUpdate = false;
							}
			...
			if ($('.rememberMe input').length > 0 && localStorage.getItem("nv-web-data") !== null) {
				...
				//check if the local storage data has expired
				if (new Date().getTime() > JSON.parse(atob(decodeURI(localStorage.getItem('nv-web-data')))).expiry) {
					localStorage.removeItem('nv-web-data');
				}
				else {
					$('#prefill-message-remember-me').show();
					$(".rememberMe input").prop("checked", true);
					NVIDIAGDC.AdaptiveForm.rememberMe = true;
					userInfo = JSON.parse(atob(decodeURI(localStorage.getItem('nv-web-data')))).value;
					
					if (guideBridge.resolveNode("firstName") !== undefined && guideBridge.resolveNode("firstName") !== null
						&& userInfo.firstName !== undefined && userInfo.firstName !== null) {
						guideBridge.resolveNode("firstName").value = userInfo.firstName;
					}
					if (guideBridge.resolveNode("lastName") !== undefined && guideBridge.resolveNode("lastName") !== null
						&& userInfo.lastName !== undefined && userInfo.lastName !== null) {
						guideBridge.resolveNode("lastName").value = userInfo.lastName;
					}
					if (guideBridge.resolveNode("userEmail") !== undefined && guideBridge.resolveNode("userEmail") !== null
						&& userInfo.userEmail !== undefined && userInfo.userEmail !== null) {
						guideBridge.resolveNode("userEmail").value = userInfo.userEmail;
						$('.userEmail input').attr('disabled', '');
					}
					if (guideBridge.resolveNode("country") !== undefined && guideBridge.resolveNode("country") !== null
						&& userInfo.country !== undefined && userInfo.country !== null) {
						guideBridge.resolveNode("country").value = userInfo.country;
					}
					if (guideBridge.resolveNode("jobTitle") !== undefined && guideBridge.resolveNode("jobTitle") !== null
						&& userInfo.jobTitle !== undefined && userInfo.jobTitle !== null) {
						guideBridge.resolveNode("jobTitle").value = userInfo.jobTitle;
					}
					if (guideBridge.resolveNode("industry") !== undefined && guideBridge.resolveNode("industry") !== null
						&& userInfo.industry !== undefined && userInfo.industry !== null) {
						guideBridge.resolveNode("industry").value = userInfo.industry;
					}
					if (guideBridge.resolveNode("company") !== undefined && guideBridge.resolveNode("company") !== null
						&& userInfo.organization !== undefined && userInfo.organization !== null) {
						guideBridge.resolveNode("company").value = userInfo.organization;
					}
				}
			}
			$('.guideFieldWidget select').each(function() {
	...
	Plugin.prototype.setFirstPartyCookies = function(userProfileData) {
		...
		if (userProfileData['id_token'] !== "undefined") {
			...
			if (Cookies.get('nvweb_S') !== "undefined" || Cookies.get('nvweb_S') !== null) {
				$.ajax({
					type: "GET",
					url: NVIDIAGDC.AdaptiveForm.eksApiServerHost + '/services/ets/v1/generate/starfleet/token',
					crossDomain: true,
					headers: {
						'Accept': 'application/json',
						'Content-Type': 'application/json',
			...
			if (Cookies.get('nvweb_E') !== "undefined" || Cookies.get('nvweb_E') !== null) {
				...
				$.ajax({
					type: "GET",
					url: NVIDIAGDC.AdaptiveForm.eksApiServerHost + '/services/ets/v1/generate/email/token?email=' + encodeURIComponent(this.aemFormloggedInuserEmail),
					crossDomain: true,
					headers: {
						'Accept': 'application/json',
						'Content-Type': 'application/json'
	...
	Plugin.prototype.getUserGeoInfo = function() {

		$.ajax({
			type: "GET",
			url: NVIDIAGDC.AdaptiveForm.eksApiServerHost + '/services/user-service/v1/users/geo/info',
			crossDomain: true,
			headers: {
				'Accept': 'application/json',
				'Content-Type': 'application/json'
			...
			success: function(data) {
				console.log("getUserGeoInfo...");
				console.log(data)
				if (data !== 'undefined') {
					if (NVIDIAGDC !== 'undefined') {
						NVIDIAGDC.addProperty("AdaptiveForm", "geoCountryCode", data.countryCode);
						NVIDIAGDC.addProperty("AdaptiveForm", "geoStateCode", data.region);
						NVIDIAGDC.addProperty("AdaptiveForm", "geoCity", data.city);
						NVIDIAGDC.addProperty("AdaptiveForm", "geoLat", data.lat);
						NVIDIAGDC.addProperty("AdaptiveForm", "geoLong", data.long);
						NVIDIAGDC.addProperty("AdaptiveForm", "geoTimeZone", data.timezone);
						NVIDIAGDC.addProperty("AdaptiveForm", "geoContinent", data.continent);
						NVIDIAGDC.addProperty("AdaptiveForm", "geoZip", data.zip);
					}
				}
			},
			error: function(data) {
	...
	Plugin.prototype.checkStarfleetLogin = function(isModalForm, formModalId, isLoginClick) {	
		return NVIDIAGDC.AdaptiveForm.checkStarfleetLogin(isModalForm, formModalId, isLoginClick);
	}
	
	NVIDIAGDC.AdaptiveForm.checkStarfleetLogin = function(isModalForm, formModalId, isLoginClick) {	
			
		var formModalIdQueryString = '';
		if(isModalForm && formModalId != undefined && formModalId !== ''){
			if (window.location.search == '') {
				formModalIdQueryString = "?modal=" + formModalId;
			} else {
				formModalIdQueryString = "&modal=" + formModalId;
			}
		}
		
		var redirectUri = window.location.href.split('/').slice(0, 3).join('/') + window.location.pathname + window.location.search + formModalIdQueryString;
		var loginRedirectUrl = NVIDIAGDC.AdaptiveForm.starfleetAuthUrl + '&ui_locales=' + guideBridge._readRuntimeLocale() + '&redirect_uri=' + encodeURIComponent(redirectUri);
		
		if(!isModalForm || (isModalForm && isLoginClick)){

			console.info("redirectURL",loginRedirectUrl);
			 // Assuming Plugin is a constructor
        	var userProfileData = Plugin.prototype.getLoggedInUserData();
			console.info("userProfileDAta",userProfileData);
			if (userProfileData === null) {
				...
				if (new Date() > new Date(userProfileData.expiration)) {
					...
					var userProfile = Plugin.prototype.parseJwtPayload(userProfileData.id_token)
					...
					Plugin.prototype.setFirstPartyCookies(userProfileData);
					$('.aemformcontainer').removeClass('hide');
				}
			}
		} 
...
FormsSubmitPlugin.prototype.handlePreFormSubmit = function($) {
	...
	if (NVIDIAGDC.AdaptiveForm.invisiblehCaptcha !== undefined && NVIDIAGDC.AdaptiveForm.invisiblehCaptcha === 'true') {
		var captchaTextHiddenField = guideBridge.resolveNode('hCaptchaToken');
		
		// Check if token already exists
		if (captchaTextHiddenField && captchaTextHiddenField.value && captchaTextHiddenField.value.trim() !== '') {
			console.log("hCaptcha token already exists, proceeding with submission");
		} else {
			console.log("Invisible hCaptcha detected, executing");
			var hcaptchaWidget = document.querySelector('.h-captcha');
			if (hcaptchaWidget) {
				try {
					if (typeof hcaptcha !== 'undefined' && hcaptcha.execute) {
						hcaptcha.execute(hcaptchaWidget);
					} else {
						console.error("hCaptcha library not loaded or execute method not available");
					}
				} catch (error) {
					console.error(error);
				}
			} else {
				console.error("hCaptcha widget not found");
			}
...
FormsSubmitPlugin.prototype.handleFormSubmit = function() {
	...
	window.guideBridge.submit({
		success: function(guideResultObject) {
			...
			$('.form-container #form-login-message').hide();
			...
			if (guideResultObject !== undefined) {
				if (guideResultObject.data.submittedToVoltron === 'true') {
					...
					if (Cookies.get('nvweb_E') !== "undefined" || Cookies.get('nvweb_E') !== null) {
						...
						$.ajax({
							type: "GET",
							url: NVIDIAGDC.AdaptiveForm.eksApiServerHost + '/services/ets/v1/generate/email/token?email=' + encodeURIComponent(guideBridge.resolveNode('userEmail').value),
							crossDomain: true,
							headers: {
								'Accept': 'application/json',
								'Content-Type': 'application/json'
					...
					/* check if on24eventRegister field if exist trigger on24 register call */
					var on24eventField = guideBridge.resolveNode("on24eventRegister");

					if (on24eventField && on24eventField !== undefined && on24eventField !== null) {

						// var dropdowns = ['country', 'industry', 'preferredLanguage', 'jobTitle'];
						// var dropdownsSelectedValues = {};
						// $('.guideFieldWidget.dropDownList').each(function() {
						// 	var selectClasses = $(this).parent().attr('class').split(' ');
						// 	var commonItem = dropdowns.filter(function(item) {
						// 		return selectClasses.includes(item);
						// 	});
						// 	if (commonItem !== null) {
						// 		dropdownsSelectedValues[commonItem] = $(this).find('.ui-selectmenu-text').text();
						// 	}
						// });

						if (NVIDIAGDC.AdaptiveForm.payloadToOn24PostSubmit !== undefined) {
							NVIDIAGDC.AdaptiveForm.payloadToOn24PostSubmit();
						}
					}
					
					//checking for rememberMe checkbox
					if (NVIDIAGDC.AdaptiveForm.rememberMe !== undefined) {
						if (NVIDIAGDC.AdaptiveForm.rememberMe === false) {
							localStorage.removeItem("nv-web-data");
						} else if (NVIDIAGDC.AdaptiveForm.rememberMe === true) {
							new FormsSubmitPlugin().createOrUpdateLocalStorage();
						}
					}
					...
					if ((enterpriseForm || developerForm || globalForm)
						...
						&& NVIDIAGDC.AdaptiveForm.userLoggedInData !== undefined
						&& !(typeof NVIDIAGDC.AdaptiveForm.skipEupUpdate !== 'undefined' && NVIDIAGDC.AdaptiveForm.skipEupUpdate === true)) {
						...
						if (guideBridge.resolveNode("country") !== undefined && guideBridge.resolveNode("country") !== null) {
							var countryCode = guideBridge.resolveNode("country").value;
						...
						if (guideBridge.resolveNode("preferredLanguage") !== undefined && guideBridge.resolveNode("preferredLanguage") !== null) {
							...
							userData.locale = locale.replace('-', '_').toLowerCase();
						} else {
							userData.locale = eupGetData.locale.replace('-', '_').toLowerCase();
						}
						...
						// 'Job_Role__c' is latest Marketo field name which is using in AEM forms
						if (guideBridge.resolveNode("Job_Role__c") !== undefined && guideBridge.resolveNode("Job_Role__c") !== null) {
							userOrgData.jobTitle = guideBridge.resolveNode("Job_Role__c").value;
						}
						...
						if (eupGetData.preferences !== undefined && eupGetData.preferences !== null) {
							eupGetData.preferences.forEach(function(preference) {
								userData.preferences.push(preference.machineName);
							});
						}

 						/*if developer on24event form submitted with interest areas then update the preferences
                         if (NVIDIAGDC.AdaptiveForm.developerRdpMembershipInterestAreasSelected !== undefined && NVIDIAGDC.AdaptiveForm.developerRdpMembershipInterestAreasSelected !== null){
							var selectedInterestAreas = NVIDIAGDC.AdaptiveForm.developerRdpMembershipInterestAreasSelected.split(',');
							if(selectedInterestAreas !== null){
								selectedInterestAreas.forEach(function(interestedAreaMachineName){
									userData.preferences.push(interestedAreaMachineName);	
								});
							}														
						} */
					...
					if (guideResultObject.data.errorData !== undefined) {
						var errorJson = null;
						try {
							errorJson = JSON.parse(decodeURIComponent(guideResultObject.data.errorData.replace(/\+/g, " ")));
						} catch (exception) {
...
FormsSubmitPlugin.prototype.createOrUpdateLocalStorage = function() {
	localStorageData = null;
	if (localStorage.getItem("nv-web-data") !== null) {
		localStorageData = JSON.parse(atob(decodeURI(localStorage.getItem("nv-web-data"))));
		userDetails = {};
		if (guideBridge.resolveNode("firstName") !== undefined && guideBridge.resolveNode("firstName") !== null && localStorageData.firstName !== guideBridge.resolveNode("firstName").value) {
			userDetails.firstName = guideBridge.resolveNode("firstName").value;
		} else if (localStorageData.firstName !== undefined) {
			userDetails.firstName = localStorageData.firstName;
		}
		if (guideBridge.resolveNode("lastName") !== undefined && guideBridge.resolveNode("lastName") !== null && localStorageData.lastName !== guideBridge.resolveNode("lastName").value) {
			userDetails.lastName = guideBridge.resolveNode("lastName").value;
		} else if (localStorageData.lastName !== undefined) {
			userDetails.lastName = localStorageData.lastName;
		}
		if (guideBridge.resolveNode("userEmail") !== undefined && guideBridge.resolveNode("userEmail") !== null && localStorageData.userEmail !== guideBridge.resolveNode("userEmail").value) {
			userDetails.userEmail = guideBridge.resolveNode("userEmail").value;
		} else if (localStorageData.userEmail !== undefined) {
			userDetails.userEmail = localStorageData.userEmail;
		}
		if (guideBridge.resolveNode("country") !== undefined && guideBridge.resolveNode("country") !== null && localStorageData.country !== guideBridge.resolveNode("country").value) {
			userDetails.country = guideBridge.resolveNode("country").value;
		} else if (localStorageData.country !== undefined) {
			userDetails.country = localStorageData.country;
		}
		if (guideBridge.resolveNode("jobTitle") !== undefined && guideBridge.resolveNode("jobTitle") !== null && localStorageData.jobTitle !== guideBridge.resolveNode("jobTitle").value) {
			userDetails.jobTitle = guideBridge.resolveNode("jobTitle").value;
		} else if (localStorageData.jobTitle !== undefined) {
			userDetails.jobTitle = localStorageData.jobTitle;
		}
		if (guideBridge.resolveNode("industry") !== undefined && guideBridge.resolveNode("industry") !== null && localStorageData.industry !== guideBridge.resolveNode("industry").value) {
			userDetails.industry = guideBridge.resolveNode("industry").value;
		} else if (localStorageData.industry !== undefined) {
			userDetails.industry = localStorageData.industry;
		}
		if (guideBridge.resolveNode("company") !== undefined && guideBridge.resolveNode("company") !== null && localStorageData.oraganization !== guideBridge.resolveNode("company").value) {
			userDetails.organization = guideBridge.resolveNode("company").value;
		} else if (localStorageData.organization !== undefined) {
			userDetails.organization = localStorageData.organization;
		}

		expiryTime = 90 * 24 * 60 * 60 * 1000;
		const now = new Date();
		const item = {
			value: userDetails,
		...
		if (localStorageData !== null) {
			if (!(_.isEqual(localStorageData, userDetails))) {
				localStorage.setItem("nv-web-data", btoa(JSON.stringify(item)));
			}

		}
	}
	else {
		//setting the local storage if it doesn't exist before
		userDetails = {};
		if (guideBridge.resolveNode("firstName") !== undefined && guideBridge.resolveNode("firstName") !== null) {
			userDetails.firstName = guideBridge.resolveNode("firstName").value;
		}
...
FormsSubmitPlugin.prototype.handleStarfleetToken = function() {
	...
	if (userProfile !== undefined && userProfile !== null && userProfile !== "" && (NVIDIAGDC.AdaptiveForm.starfleetAuthRequired === 'true' || NVIDIAGDC.AdaptiveForm.jarvisAuthRequired === 'true')) {
...
let optinTopicsMap = JSON.parse('{"TX Topic - Virtual Reality": "virtual_reality","TX Topic - Computer Vision": "computer_vision","TX Topic - Cybersecurity": "cybersecurity","TX Topic - MLOps": "mlops","TX Topic - Graphics": "graphics","TX Topic - Game Engines": "game_engines","TX Topic - Generative AI": "generative_ai","TX Topic - Networking / Communications": "networking","TX Topic - Conversational AI": "conversational_ai","TX Topic - Data Science": "recommenders","TX Topic - Robotics": "robotics","TX Topic - Simulation": "simulation","TX Topic - Video Streaming": "video_streaming","TX Topic - Virtualization": "virtualization","TX Topic - Other": "other","TX Topic - Data Center": "data_center","TX Topic - AI Platforms / Deployment": "deployment","TX Topic - Development & Optimization": "development","TX Topic - Models / Libraries / Frameworks": "models"}');
